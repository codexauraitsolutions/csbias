...
const EMPTY = { group: "why_choose_us", title: "", description: "", imageUrl: "", linkUrl: "", order: 0, status: "draft" };

const GROUPS = [
  { value: "latest_updates", label: "Latest Updates (ticker)" },
  { value: "why_choose_us", label: "Why Choose Us" },
  { value: "exam_stages", label: "Exam Stages" },
  { value: "locations", label: "Branches / Locations" },
];

export default function HighlightsList() {
  const [items, setItems] = useState(null);
  const [activeGroup, setActiveGroup] = useState("why_choose_us");
  ...
  function load() {
    api.highlights.list().then(setItems);
  }
  useEffect(load, []);
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      if (editingId) {
        await api.highlights.update(editingId, payload);
      } else {
        await api.highlights.create(payload);
      }
      setForm({ ...EMPTY, group: activeGroup });
      setEditingId(null);
      load();
    } finally {
      setSaving(false);
  ...
  const filtered = items
    ? items.filter(
        (i) => i.group === activeGroup && (i.title || "").toLowerCase().includes(search.trim().toLowerCase())
      )
    : [];
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <h1 className="text-2xl font-bold">Homepage Content Blocks</h1>

      <div className="flex gap-2">
        {GROUPS.map((g) => (
          <button
            key={g.value}
            onClick={() => { setActiveGroup(g.value); setForm({ ...EMPTY, group: g.value }); setEditingId(null); }}
            className={`px-3 py-1.5 rounded text-sm ${activeGroup === g.value ? "bg-indigo-600 text-white" : "bg-gray-100"}`}
          >
            {g.label}
          </button>
        ))}
      ...
      <form onSubmit={handleSave} className="bg-white rounded-lg shadow p-6 space-y-4">
        ...
        <div className="flex gap-3">
          ...
          {editingId && (
            <button type="button" onClick={() => { setEditingId(null); setForm({ ...EMPTY, group: activeGroup }); }} className="px-4 py-2 rounded-md border text-sm font-medium hover:bg-gray-50">
              Cancel
            </button>
          )}
        </div>
      ...
      <input
        placeholder="Search this group by title…"
      ...
      <div className="bg-white rounded-lg shadow divide-y">
        {pageItems.map((item) => (
          <div key={item.id} className="flex items-center gap-4 px-5 py-3">
            {item.imageUrl && <img src={absoluteUrl(item.imageUrl)} alt="" className="w-10 h-10 object-contain" />}
        ...
        {filtered.length === 0 && <p className="px-5 py-8 text-center text-gray-400">No items in this group yet.</p>}
      </div>
